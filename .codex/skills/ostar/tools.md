# Objectstar Shareable Tools Reference

Built-in functions available in all rules. Call without CALL statement.
...
| `HOUR` | Current hour | Returns 0-23 |
| `MINUTE` | Current minute | Returns 0-59 |
| `SECOND` | Current second | Returns 0-59 |
...
| Function | Description | Example |
|----------|-------------|---------|
| `$GET_TYPE(v)` | Get semantic type | Returns C/D/I/L/Q/S |
| `$GET_SYNTAX(v)` | Get syntax type | Returns B/C/P/V/F/UN |
| `$GET_SIZE(v)` | Get byte size | Returns integer |
| `$TYPECAST(type,syn,size,dec,val)` | Type conversion | Cast value to type |
| `$ISNULL(v)` | Check if null | Returns Y/N |
...
**Common options**:
- `COMMITUSED` - Intent list entries used
- `COMMITSIZE` - Intent list maximum
- `MAXLOCK` - Maximum locks
- `TRACE` - Trace level
...
### String Manipulation
```
LOCAL FULL_NAME, UPPER_NAME, INITIALS;
FULL_NAME = EMPLOYEES.FNAME || ' ' || EMPLOYEES.LNAME;
UPPER_NAME = UPPERCASE(FULL_NAME);
...
```
LOCAL VAR_TYPE;
VAR_TYPE = $GET_TYPE(INPUT_VALUE);
VAR_TYPE = 'C';                      | Y N
------------------------------------+-----
CALL PROCESS_COUNT(INPUT_VALUE);    | 1
CALL PROCESS_OTHER(INPUT_VALUE);    |   1
```
